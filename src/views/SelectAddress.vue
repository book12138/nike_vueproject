<template>
  <div>
    ...
    <div class="addressMoudles" v-for="item in addresses" :key="item.id">
      <div class="contentContainer" @click="select(item.id)">
      ...
      <div class="userPlayContainer">
        <span @click="addressItemClickHandle(item.id)">
...
export default {
  ...
  methods: {
    ...
    /** 用户选中地址 */
    select (id) {
      sessionStorage.setItem('userSelectOrderAddress', id)
      this.back()
    }
  }
}
