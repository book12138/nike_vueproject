<template>
  <div style="margin-top:120px;text-align:left;margin-bottom:51px;">
    <Navigation :flag="4"></Navigation>
    <img
      v-show="evaluateOrderInfo.length === 0"
      src="@/assets/icon/空空如也.png"
      style="position:absolute;top:150px;margin:0px calc(50% - 55px);z-index:99;width:100px;"
    />

    <!-- 订单列表 -->
    <div
      v-for="info in evaluateOrderInfo"
      :key="info.id"
    >
      <div class="ordersku-item" v-for="item in info.orderInfos" :key="item.skuId">
        ...
        <div class="ordersku-tool">
          <div class="ordersku-tool-btn fl_right" @click="handleEvaluate(info.id,item.skuId)">立即评价</div>
          <div class="clr"></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  created () {
    ...
    this.$axios
      .get(`${api.order}/api/EvaluateOrder`)
      .then(success => {
        this.evaluateOrderInfo = success.data
        this.appendSkuInfo()
      })
      .catch(error => {
  ...
  methods: {
    ...
    appendSkuInfo () {
      this.evaluateOrderInfo.forEach(element => {
        element.orderInfos.forEach(ele => {
          ...
          this.$axios
            ...
            .then(successResponse => {
              this.evaluateOrderInfo.forEach(t => {
    ...
    handleEvaluate (orderId, skuId) {
      sessionStorage.setItem('evaluate_orderId', orderId)
      sessionStorage.setItem('evaluate_skuId', skuId)
