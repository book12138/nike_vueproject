<template>
  <div class="paymentContainer">
    ...
    <div class="paymentTitle">
      <div class="paymentTitle-content">
        <div class="paymentTitle-content-text fl_left">
          <span style="font-size:20px;">等待付款</span>
          <p style="font-size:12px;">自订单创建起，三十分钟内未付款，订单将自动取消</p>
        </div>
    ...
    <!-- 收货地址 -->
    <div class="payment-address" @click="addressClickHandle">
      <img
        src="@/assets/icon/收货地址1.png"
        style="height:35px;margin:5px 20px 0px 10px;"
        class="fl_left"
      />
      <div class="fl_left" style="width:calc(90% - 80px);">
    ...
    <div class="payment-tool">
      <div class="payment-tool-btn fl_right" style="border-color: #e25e3a;" @click="commit">付款</div>
      <div class="payment-tool-btn fl_right" @click="orderCancel">取消订单</div>
      <div class="clr"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    if (sessionStorage.getItem('userSelectOrderAddress')) {
      // 用户修改了地址
      this.$axios.put(`${api.order}/api/Order`, {
        id: sessionStorage.getItem('orderId'),
        shippingAddressId: sessionStorage.getItem('userSelectOrderAddress')
      }).then(ref => {
        this.$axios
          .get(
            `${api.order}/api/PaymentOrder/id?id=${sessionStorage.getItem(
              'orderId'
            )}`
          )
          .then(success => {
    ...
    } else if (sessionStorage.getItem('orderId')) {
      this.$axios
        .get(
          `${api.order}/api/PaymentOrder/id?id=${sessionStorage.getItem(
            'orderId'
  ...
  methods: {
    ...
    orderCancel () {
      this.$axios.put(`${api.order}/api/Order/id?id=${sessionStorage.getItem('orderId')}`).then(success => {
    ...
    commit () {
      this.$axios
        .put(`${api.order}/api/PaymentOrder?id=${sessionStorage.getItem('orderId')}`)
        .then(success => {
          if (success.data.success) {
            this.$router.replace({ name: 'nonShipmentsOrder' })
          }
        }).catch(error => {
          if (!auth.catchInfoFileter(error, true)) {
            this.$message.error('对于你的请求处理出现了错误')
