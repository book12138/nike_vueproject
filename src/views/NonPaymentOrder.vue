<template>
  <div style="margin-top:120px;text-align:left;margin-bottom:51px;">
    ...
    <!-- 订单列表 -->
    <div
      @click="redirectToPayment(info.id)"
      class="ordersku-item-container"
      v-for="info in orderInfo"
      :key="info.id"
    >
      <div style="width:100%;border-left:2px solid red;text-indent:10px;margin-left:2px;">待处理订单</div>
      ...
      <div class="ordersku-item" v-for="item in info.orderInfos" :key="item.skuId">
        ...
        <div class="ordersku-item-info fl_left">
          <p class="ordersku-item-title">{{item.skuInfo[0].title}}</p>
          <div
      ...
      <div class="ordersku-price">
        共{{ getNum(info) }}件商品，合计
        <span style="color:red">￥{{ info.total}}</span>
      </div>

      <!-- 按钮组 -->
      <div class="ordersku-tool">
        <div
          class="ordersku-tool-btn fl_right"
          style="border-color: #e25e3a;"
          @click.self="commit(info.id)"
        >付款</div>
        <div class="ordersku-tool-btn fl_right" @click="cancel(info.id)">取消订单</div>
        <div class="clr"></div>
      </div>
    </div>
...
export default {
  ...
  created () {
    ...
    this.$axios
      .get(`${api.order}/api/PaymentOrder`)
      .then(success => {
  ...
  methods: {
        ...
        element.orderInfos.forEach(ele => {
          ...
          this.$axios
            ...
            .then(successResponse => {
              // console.log(ele)
              // this.skuInfos.push(successResponse.data)
              // var temp = ''
              // var old = JSON.stringify(ele)
              // old = old.substring(1, old.length - 1)
              // var appendInfo = JSON.stringify({
              //   skuinfo: successResponse.data
              // })
              // appendInfo = appendInfo.substring(1, appendInfo.length - 1)
              // temp = `{${old},${appendInfo}}`
              // target = JSON.parse(temp)
    ...
    getNum (info) {
      let num = 0
      info.orderInfos.forEach(element => {
    ...
    cancel (id) {
      this.$axios.put(`${api.order}/api/Order/id?id=${id}`).then(success => {
    ...
    commit (id) {
      this.$axios
        .put(`${api.order}/api/PaymentOrder?id=${id}`)
        .then(success => {
          if (success.data.success) {
            this.$router.push({ name: 'nonShipmentsOrder' })
          }
        })
    },
    showDetails (id) {
